var p=Object.defineProperty;var c=(n,t,e)=>t in n?p(n,t,{enumerable:!0,configurable:!0,writable:!0,value:e}):n[t]=e;var o=(n,t,e)=>c(n,typeof t!="symbol"?t+"":t,e);self.importScripts("https://cdn.jsdelivr.net/pyodide/v0.26.4/full/pyodide.js");class u{constructor(){o(this,"pyodide",null);o(this,"isInitialized",!1);o(this,"initPromise",null);o(this,"stdoutBuffer",[]);o(this,"stderrBuffer",[]);o(this,"interruptBuffer",null);self.addEventListener("message",this.handleMessage.bind(this))}async handleMessage(t){const e=t.data;try{switch(e.type){case"init":await this.initialize(e.indexURL,e.packages);break;case"execute":await this.execute(e);break;case"loadPackages":await this.loadPackages(e.packages);break;case"setupNLTK":await this.setupNLTK(e.dataPath);break;case"interrupt":this.interrupt();break;default:this.sendError("Unknown message type",void 0)}}catch(s){this.sendError(s instanceof Error?s.message:"Unknown error",e.id)}}async initialize(t,e){if(this.isInitialized){this.send({type:"ready"});return}if(this.initPromise){await this.initPromise;return}this.initPromise=this.doInitialize(t,e),await this.initPromise}async doInitialize(t,e){try{const s=i=>{this.stdoutBuffer.push(i),this.send({type:"stdout",data:i})},d=i=>{this.stderrBuffer.push(i),this.send({type:"stderr",data:i})};this.pyodide=await loadPyodide({stdout:s,stderr:d,fullStdLib:!1}),self.crossOriginIsolated&&(this.interruptBuffer=new Uint8Array(new SharedArrayBuffer(1)),this.pyodide.setInterruptBuffer(this.interruptBuffer));const l=e||["micropip","numpy","regex"];try{await this.pyodide.loadPackage(l)}catch(i){console.error("Failed to load packages:",i)}await this.pyodide.runPythonAsync(`
        import micropip
        import sys
        import json
        import traceback
      ...
      `),this.send({type:"nltk_ready"})}catch(e){throw new Error(`NLTK setup failed: ${e}`)}}async execute(t){if(!this.pyodide)throw new Error("Pyodide not initialized");const{id:e,code:s,functionName:d,args:l}=t;try{this.stdoutBuffer=[],this.stderrBuffer=[],await this.pyodide.loadPackagesFromImports(s),this.pyodide.globals.set("_parser_input",l[0]);const i=`
import sys
import json
import traceback
import io
...
_parser_result
      `,a=await this.pyodide.runPythonAsync(i);this.pyodide.globals.delete("_parser_input");let r=a;a&&typeof a.toJs=="function"&&(r=a.toJs({dict_converter:Object.fromEntries})),r.stdout&&this.send({type:"stdout",id:e,data:r.stdout}),r.stderr&&this.send({type:"stderr",id:e,data:r.stderr}),r.success?this.send({type:"result",id:e,data:r.value}):(console.error("Python execution error:",{type:r.error_type,message:r.error_message,traceback:r.error}),this.send({type:"error",id:e,error:r.error||r.error_message||"Unknown error"}))}catch(i){let a=i instanceof Error?i.message:String(i);if(this.pyodide)try{const r=await this.pyodide.runPythonAsync(`
import sys
import traceback
...
error_msg
          `);r&&(a=r)}catch{}console.error("Python execution error:",a),this.send({type:"error",id:e,error:a})}}interrupt(){this.interruptBuffer&&this.pyodide?(this.interruptBuffer[0]=2,this.send({type:"result",data:"Interrupted"})):this.sendError("Interrupts not available (requires cross-origin isolation)",void 0)}send(t){self.postMessage(t)}sendError(t,e){this.send({type:"error",id:e,error:t})}}new u;

var p=Object.defineProperty;var y=(d,t,e)=>t in d?p(d,t,{enumerable:!0,configurable:!0,writable:!0,value:e}):d[t]=e;var o=(d,t,e)=>y(d,typeof t!="symbol"?t+"":t,e);self.importScripts("https://cdn.jsdelivr.net/pyodide/v0.26.4/full/pyodide.js");class h{constructor(){o(this,"pyodide",null);o(this,"isInitialized",!1);o(this,"initPromise",null);o(this,"stdoutBuffer",[]);o(this,"stderrBuffer",[]);o(this,"interruptBuffer",null);o(this,"memoryMonitorInterval",null);o(this,"executionCount",0);self.addEventListener("message",this.handleMessage.bind(this))}async handleMessage(t){const e=t.data;try{switch(e.type){case"init":await this.initialize(e.indexURL,e.packages);break;case"execute":await this.execute(e);break;case"loadPackages":await this.loadPackages(e.packages);break;case"setupNLTK":await this.setupNLTK(e.dataPath);break;case"interrupt":this.interrupt();break;default:this.sendError("Unknown message type",void 0)}}catch(i){this.sendError(i instanceof Error?i.message:"Unknown error",e.id)}}async initialize(t,e){if(this.isInitialized){this.send({type:"ready"});return}if(this.initPromise){await this.initPromise;return}this.initPromise=this.doInitialize(t,e),await this.initPromise}async doInitialize(t,e){try{const i=r=>{this.stdoutBuffer.push(r),this.send({type:"stdout",data:r})},a=r=>{this.stderrBuffer.push(r),this.send({type:"stderr",data:r})};this.pyodide=await loadPyodide({stdout:i,stderr:a,fullStdLib:!1}),self.crossOriginIsolated&&(this.interruptBuffer=new Uint8Array(new SharedArrayBuffer(1)),this.pyodide.setInterruptBuffer(this.interruptBuffer));const c=e||["micropip","numpy","regex"];console.log("Loading micropip, numpy, packaging, regex");try{await this.pyodide.loadPackage(c),console.log("Packages loaded successfully")}catch(r){console.error("Failed to load packages:",r)}await this.pyodide.runPythonAsync(`
        import micropip
        import sys
        import json
        import traceback
      ...
      `),this.send({type:"nltk_ready"})}catch(e){throw new Error(`NLTK setup failed: ${e}`)}}async execute(t){if(!this.pyodide)throw new Error("Pyodide not initialized");const{id:e,code:i,functionName:a,args:c}=t;try{this.stdoutBuffer=[],this.stderrBuffer=[],await this.pyodide.loadPackagesFromImports(i);const r=`_parser_input_${e}`;try{const u=c[0];if(this.pyodide.globals.set(r,u),this.pyodide.globals.get(r)===void 0)throw new Error(`Failed to verify ${r} was set in globals`)}catch(u){throw console.error(`Failed to set ${r} in Pyodide globals:`,u,"args[0]:",c[0]),new Error(`Failed to set parser input: ${u}`)}const l=`
import sys
import json
import traceback
import io
...
_parser_result
      `,n=await this.pyodide.runPythonAsync(l);this.pyodide.globals.delete(r);let s=n;n&&typeof n.toJs=="function"&&(s=n.toJs({dict_converter:Object.fromEntries})),s.stdout&&this.send({type:"stdout",id:e,data:s.stdout}),s.stderr&&this.send({type:"stderr",id:e,data:s.stderr}),s.success?this.send({type:"result",id:e,data:s.value}):(console.error("Python execution error:",{type:s.error_type,message:s.error_message,traceback:s.error}),this.send({type:"error",id:e,error:s.error||s.error_message||"Unknown error"})),this.cleanupExecution(e)}catch(r){let l=r instanceof Error?r.message:String(r);if(this.pyodide)try{const n=await this.pyodide.runPythonAsync(`
import sys
import traceback
...
error_msg
          `);n&&(l=n)}catch{}console.error("Python execution error:",l),this.send({type:"error",id:e,error:l}),this.cleanupExecution(e)}}interrupt(){this.interruptBuffer&&this.pyodide?(this.interruptBuffer[0]=2,this.send({type:"result",data:"Interrupted"})):this.sendError("Interrupts not available (requires cross-origin isolation)",void 0),this.memoryMonitorInterval&&(clearInterval(this.memoryMonitorInterval),this.memoryMonitorInterval=null)}send(t){self.postMessage(t)}sendError(t,e){this.send({type:"error",id:e,error:t})}startMemoryMonitoring(){this.memoryMonitorInterval=self.setInterval(()=>{this.checkMemoryUsage()},3e4),this.executionCount%10===0&&this.checkMemoryUsage()}async checkMemoryUsage(){try{if(performance.memory){const t=performance.memory.usedJSHeapSize,e=t/1e6;console.log(`Pyodide Worker Memory: ${e.toFixed(2)}MB`),t>5e7&&this.pyodide&&(console.log("Running Python garbage collection..."),await this.pyodide.runPythonAsync(`
            import gc
            collected = gc.collect()
            print(f"Python GC collected {collected} objects")
          `)),t>1e8&&(console.warn(`High memory usage in Pyodide worker: ${e.toFixed(2)}MB`),this.send({type:"error",error:`HIGH_MEMORY: ${e.toFixed(2)}MB - Consider recycling worker`}))}}catch(t){console.error("Memory monitoring error:",t)}}cleanupExecution(t){const e=this.stdoutBuffer.filter(a=>!a.includes(t)),i=this.stderrBuffer.filter(a=>!a.includes(t));this.stdoutBuffer=e.slice(-100),this.stderrBuffer=i.slice(-100),this.executionCount++}}new h;

const __vite__mapDeps=(i,m=__vite__mapDeps,d=(m.f||(m.f=["assets/jszip.min-2K5cafnC.js","assets/ui-vendor-DhLsVKdZ.js","assets/vue-vendor-BQHtwQ0z.js","assets/utils-vendor-D3BN4yKY.js","assets/data-vendor-CJedmZld.js"])))=>i.map(i=>d[i]);
var A=Object.defineProperty;var I=(p,e,r)=>e in p?A(p,e,{enumerable:!0,configurable:!0,writable:!0,value:r}):p[e]=r;var w=(p,e,r)=>I(p,typeof e!="symbol"?e+"":e,r);import{l as c,_ as j}from"./index-C88oTStw.js";import{t as C,E,g as R,d as S}from"./TrialsView-Dsz0uR11.js";import{TrialBundleExportService as D}from"./trial-bundle.service-BAq1Lc6S.js";import"./vue-vendor-BQHtwQ0z.js";import"./ui-vendor-DhLsVKdZ.js";import"./utils-vendor-D3BN4yKY.js";import"./tauri-vendor-1uBLmS9u.js";import"./toast-CV1Vu7QO.js";import"./useCostEstimation-8hMN6UGJ.js";import"./useModels-CRcY4pG8.js";import"./useLiveQuery-BiK0jbRJ.js";import"./ParserSelector-CGlQJGlq.js";import"./parser.store-DhAH9c0o.js";import"./trial-stream-manager-LDVobxnO.js";import"./execution-control-DaE3TqkV.js";import"./index-Brb0k-N6.js";import"./index-BSqJbMhU.js";import"./id-generator-Cv_Imc0S.js";import"./useTrialCommands-DJTDlE53.js";import"./variable-list.repository-CNU9Up7O.js";import"./template.repository-Dc8Igzx0.js";import"./useHeatmapStyling-Dm6BuuHr.js";import"./sanitization-C_Jjj85o.js";import"./data-vendor-CJedmZld.js";import"./useTrials-rbSU1oxY.js";class O{getTemplate(){return{id:"minimal",name:"Minimal Experiment Script",description:"Minimal Python script with clear experimental design and pre-built requests",pythonVersion:"3.8",requiredImports:["json","requests","itertools","csv"],templateCode:"MINIMAL_TEMPLATE",supportsAnalysis:!1,supportsVisualization:!1}}generateScript(e,r,t={}){const{includeComments:s=!0}=t,n=this.buildResolvedConfigurations(e);return[this.generateHeader(e,s),this.generateImports(),this.generateExperimentSection(e,s),this.generateResolvedConfigsSection(n,s),this.generateMinimalUtilities(),this.generateExecutionSection(e,s)].filter(Boolean).join(`
...
def show_summary(results_file):
    ...
    print(f"{'='*50}\\n")`}toPythonJson(e){return JSON.stringify(e,null,4).replace(/: false/g,": False").replace(/: true/g,": True").replace(/: null/g,": None")}}function N(p){switch(p){case"minimal":case"basic":case"reproduction":case"analysis":return new O;default:throw new Error(`Unknown template: ${p}`)}}function G(){return[new O().getTemplate()]}class ${async generateScript(e,r={}){const t=Date.now();c.info("EXPORT_DEBUG: PYTHON_GENERATION START",{trialId:e,template:r.template||"minimal",startTime:t});try{c.info("EXPORT_DEBUG: PYTHON_GENERATION LOADING_TRIAL",{trialId:e});const s=await C.loadTrialForExport(e);if(!s.ok){const b=s.error;return{ok:!1,error:new E("GENERATION_FAILED",`Failed to load trial data: ${b.message}`,{trialId:e,originalError:b})}}const{trial:n,apiCalls:a}=s.value,i=this.validateCodeGenOptions(r);if(!i.isValid)return{ok:!1,error:new E("INVALID_FIELDS",`Code generation validation failed: ${i.errors.join(", ")}`,{errors:i.errors})};const o=r.template||"minimal";c.info("EXPORT_DEBUG: PYTHON_GENERATION GETTING_TEMPLATE",{templateId:o});const g=N(o),f=g.getTemplate();c.info("EXPORT_DEBUG: PYTHON_GENERATION GENERATING_SCRIPT",{templateId:o,apiCallCount:a.length});const u=Date.now(),d=g.generateScript(n,a,{includeComments:r.includeComments!==!1});c.info("EXPORT_DEBUG: PYTHON_GENERATION SCRIPT_GENERATED",{durationMs:Date.now()-u,codeLength:d.length,estimatedBytes:R(d)});const m=Date.now()-t,_={filename:this.generateScriptFilename(n.name,o),mimeType:"text/x-python",data:d,metadata:{trialName:n.name,trialId:n.id,exportDate:new Date,callCount:a.length,fieldCount:0,format:"python",estimatedSizeBytes:R(d)},template:o,pythonVersion:f.pythonVersion,requiredPackages:this.extractRequiredPackages(f,r),codeLength:d.length,generationTimeMs:m};return c.info("EXPORT_DEBUG: PYTHON_GENERATION SUCCESS",{trialId:e,templateId:o,filename:_.filename,codeLength:_.codeLength,generationTimeMs:_.generationTimeMs,requiredPackages:_.requiredPackages}),{ok:!0,value:_}}catch(s){const n=s instanceof Error?s:new Error(String(s));return c.info("EXPORT_DEBUG: PYTHON_GENERATION UNEXPECTED_ERROR",n,{trialId:e,template:r.template||"minimal",generationTimeMs:Date.now()-t}),{ok:!1,error:new E("GENERATION_FAILED",`Python script generation failed: ${n.message}`,{trialId:e,options:r,originalError:s,generationTimeMs:Date.now()-t})}}}getAvailableTemplates(){return G()}validateCodeGenOptions(e){const r=[],t=[];if(e.template){const s=this.getAvailableTemplates().map(n=>n.id);s.includes(e.template)||r.push(`Invalid template: ${e.template}. Available templates: ${s.join(", ")}`)}if(e.pythonVersion){const s=["3.8","3.9","3.10","3.11"];s.includes(e.pythonVersion)||t.push(`Python version ${e.pythonVersion} not explicitly supported. Supported versions: ${s.join(", ")}`)}if(e.includeVisualization){const s=e.template||"minimal";N(s).getTemplate().supportsVisualization||t.push(`Template '${s}' does not support built-in visualization. You can add your own visualization code to the exported script.`)}return{isValid:r.length===0,errors:r,warnings:t}}generateScriptFilename(e,r){const t=e.replace(/[^a-zA-Z0-9\s-_]/g,"").replace(/\s+/g,"_").toLowerCase().substring(0,30).replace(/^_+|_+$/g,"")||"trial",s=r==="minimal"?"":`_${r}`,n=new Date().toISOString().split("T")[0];return`${t}${s}_${n}.py`}extractRequiredPackages(e,r){return["json","requests","itertools","csv"]}}const B=new $;let k=null;async function F(){return k||(k=(await j(async()=>{const{default:p}=await import("./jszip.min-2K5cafnC.js").then(e=>e.j);return{default:p}},__vite__mapDeps([0,1,2,3,4]))).default),k}class L{constructor(){w(this,"trialBundleService",new D)}async exportBundle(e,r){try{c.info("EXPORT_DEBUG: REPRO_BUNDLE START",{trialId:e,bundleType:r.bundleType});const t=await B.generateScript(e,{template:"minimal",includeComments:r.includeComments!==!1});if(!t.ok)return{ok:!1,error:new E("GENERATION_FAILED",t.error?.message||"Failed to generate Python script")};if(r.bundleType==="script-only")return{ok:!0,value:{zipBlob:new Blob([t.value.data],{type:"text/x-python"}),filename:t.value.filename,sizeBytes:t.value.metadata.estimatedSizeBytes,contents:["experiment.py"]}};const s=await F(),n=new s,a=[];n.file("experiment.py",t.value.data),a.push("experiment.py");const i=await this.trialBundleService.exportTrialBundle(e,{includeResults:!0,includeApiCalls:!0,skipDownload:!0});if(i.ok?(n.file("trial_config.json",JSON.stringify(i.value.bundle,null,2)),a.push("trial_config.json")):c.warn("Failed to export trial bundle:",i.error),r.bundleType==="script-config-data"){const m=n.folder("data"),h=await S.getAvailableFields(e);let _=[];if(h.ok){const y=h.value.standardFields.map(l=>l.id).filter(l=>l!=="parameters"&&l!=="variables"),x=h.value.parameterFields.map(l=>l.id),v=(h.value.variableFields||[]).filter(l=>!l.id.startsWith("source_")).map(l=>l.id);_=[...y,...x,...v],c.info("EXPORT_DEBUG: REPRO_BUNDLE using all fields",{total:_.length,standard:y.length,params:x.length,vars:v.length})}else c.warn("Failed to get available fields, using defaults");const b=[{format:"csv",filename:"results.csv"},{format:"excel",filename:"results.xlsx"},{format:"jsonl",filename:"results.jsonl"}];for(const{format:y,filename:x}of b){const v=await S.exportTrialData(e,{format:y,fields:_.length>0?_:void 0,includeHeaders:!0,addBOM:y==="csv"||y==="excel"});if(v.ok){if(y==="excel"&&typeof v.value.data=="string"){const l=atob(v.value.data),P=new Uint8Array(l.length);for(let T=0;T<l.length;T++)P[T]=l.charCodeAt(T);m.file(x,P)}else m.file(x,v.value.data);a.push(`data/${x}`)}else c.warn(`Failed to export ${y}:`,v.error)}}const o=this.generateReadme(r.bundleType,t.value.metadata.trialName);n.file("README.md",o),a.push("README.md");const g=await n.generateAsync({type:"blob",compression:"DEFLATE",compressionOptions:{level:6}}),f=new Date().toISOString().split("T")[0].replace(/-/g,""),u=r.bundleType==="script-config-data"?"_complete":"_config",d=`${t.value.metadata.trialName.replace(/[^a-zA-Z0-9_-]/g,"_")}_repro${u}_${f}.zip`;return{ok:!0,value:{zipBlob:g,filename:d,sizeBytes:g.size,contents:a}}}catch(t){return c.error("EXPORT_DEBUG: REPRO_BUNDLE ERROR",t),{ok:!1,error:new E("GENERATION_FAILED",t instanceof Error?t.message:"Unknown error creating bundle")}}}generateReadme(e,r){const t=new Date().toISOString();let s="";switch(e){case"script-config":s="\n## Contents\n- `experiment.py` - Minimal Python script to reproduce the experiment\n- `trial_config.json` - Complete trial configuration including templates, variables, and model settings\n";break;case"script-config-data":s="\n## Contents\n- `experiment.py` - Minimal Python script to reproduce the experiment  \n- `trial_config.json` - Complete trial configuration\n- `data/results.csv` - Full experimental results in CSV format\n- `data/results.xlsx` - Full experimental results in Excel format\n- `data/results.jsonl` - Full experimental results in JSONL format (one JSON object per line)\n";break}return`# Reproducibility Bundle: ${r}
...
`}}const me=new L;export{me as reproducibilityBundleService};

...
var O=Object.defineProperty;var C=(f,e,r)=>e in f?O(f,e,{enumerable:!0,configurable:!0,writable:!0,value:r}):f[e]=r;var k=(f,e,r)=>C(f,typeof e!="symbol"?e+"":e,r);import{l as _,_ as M}from"./index-DKYdGRb4.js";import{t as L,E as x,g as A,d as R}from"./TrialsView-BTRmQY26.js";import{TrialBundleExportService as q}from"./trial-bundle.service-B2UuGtr6.js";import"./vue-vendor--FHcN8mI.js";import"./ui-vendor-BXvbAVk1.js";import"./utils-vendor-C2jMmoQd.js";import"./tauri-vendor-1uBLmS9u.js";import"./toast-C8a3oI2o.js";import"./useCostEstimation-DXc_fn-S.js";import"./useModels-ByPD-z0G.js";import"./useLiveQuery-C6RixDNq.js";import"./ParserSelector-CPdNIf61.js";import"./parser-CoIZpemd.js";import"./stream-manager-R0uCLHHc.js";import"./controller-DkNQkulq.js";import"./common-Brb0k-N6.js";import"./index-D9kY9InN.js";import"./id-generator-Cv_Imc0S.js";import"./useTrialCommands-DNuCvnvj.js";import"./repository-D3NF8n7M.js";import"./useTrials-C1Heg4Zq.js";import"./repository-B2kbGz9s.js";import"./useHeatmapStyling-bPN7fji7.js";import"./sanitization-G2YsPkpv.js";import"./data-vendor-BRSVhGEn.js";class S{getTemplate(){return{id:"simple",name:"Simple Script (Beginner-Friendly)",description:"Simple Python script with CSV/JSON output, basic retries, and clear progress. Best for beginners and quick tests.",pythonVersion:"3.8",requiredImports:["json","requests","os","time","datetime"],templateCode:"SIMPLE_TEMPLATE",supportsAnalysis:!1,supportsVisualization:!1}}generateScript(e,r,s={}){const{includeComments:t=!0}=s,o=this.buildResolvedConfigurations(e),a=this.discoverParameterColumns(o);return[this.generateHeader(e,t),this.generateImports(),this.generateSettings(e,t),this.generateParameterColumns(a,t),this.generateModelConfigs(o,t),this.generateFunctions(),this.generateMainFunction(e,a),this.generateExecutor()].filter(Boolean).join(`

`)}buildResolvedConfigurations(e){const r=[],s=new Map,t=new Map;for(const o of e.configurations){const a=o.modelSnapshot?.displayName||o.modelId;s.set(a,(s.get(a)||0)+1),t.set(a,0)}for(let o=0;o<e.configurations.length;o++){const a=e.configurations[o],n=a.providerSnapshot;if(!n)continue;const p=n.api?.baseUrl||"https://api.example.com",u=n.api?.endpoint||"/v1/api",l={"Content-Type":"application/json",...n.headers};n.auth?.type==="bearer"?l.Authorization="Bearer {{API_KEY}}":n.auth?.type==="header"&&n.auth.header&&(l[n.auth.header]="{{API_KEY}}");const d={model:a.modelId},i=n.bodyConstruction;i?.promptField?i.wrapAsArray?d[i.promptField]=[{role:i.messageRole||"user",content:"{{PROMPT}}"}]:d[i.promptField]="{{PROMPT}}":d.prompt="{{PROMPT}}",Object.assign(d,a.parameters);const h=a.provider.split("-")[0],c=a.modelSnapshot?.displayName||a.modelId;let m=c;if(s.get(c)>1){const y=t.get(c)+1;t.set(c,y),m=`${c} [Config ${y}]`}r.push({config_index:o,name:c,display_name:m,provider:h,url:p+u,headers:l,body:d,parameters:a.parameters||{},extract_paths:this.getExtractPaths(a)})}return r}getExtractPaths(e){const r=e.providerSnapshot;if(!r)return["response"];const s=r.responseModes?.text;if(!s)return["response"];const t=s.responseTransform;if(!t)return["response"];const o=[];return t.contentPath&&o.push(t.contentPath),t.fallbackPaths&&o.push(...t.fallbackPaths),o.length>0?o:["response"]}discoverParameterColumns(e){const r=new Set;for(const s of e)s.parameters&&Object.keys(s.parameters).forEach(t=>r.add(t));return Array.from(r).sort()}generateParameterColumns(e,r){const s=r?`# All unique parameters found across model configurations
`:"",t=JSON.stringify(e);return`${s}PARAMETER_COLUMNS = ${t}`}generateHeader(e,r){const s="#!/usr/bin/env python3";return r?`${s}
"""
...
import os
import argparse`}generateSettings(e,r){const s=r?`# ============================================
# YOUR EXPERIMENT SETTINGS
# ============================================

`:"",t=new Set;for(const n of e.configurations){const p=n.provider.split("-")[0];t.add(p)}const o=[];for(const n of t)if(n==="ollama")o.push(`    "${n}": "",  # Local, no key needed`);else{const p=`${n.toUpperCase()}_API_KEY`;o.push(`    "${n}": os.getenv("${p}", ""),  # Get from environment or set here`)}let a="";if(e.type==="template"&&e.templateConfig){const{template:n,variables:p}=e.templateConfig,u={};for(const[d,i]of Object.entries(p))"snapshot"in i&&i.snapshot?u[d]=i.snapshot.items.map(h=>h.value):i.type==="value"&&i.values&&(u[d]=[...i.values]);const l=JSON.stringify(u,null,4).replace(/: false/g,": False").replace(/: true/g,": True").replace(/: null/g,": None");a=`
...
${a}`}generateModelConfigs(e,r){const s=r?`# ============================================
# MODELS TO TEST (exported from Auditomatic)
# ============================================

`:"",t=e.map(a=>({config_index:a.config_index,name:a.name,display_name:a.display_name,provider:a.provider,url:a.url,headers:a.headers,body:a.body,parameters:a.parameters,extract_paths:a.extract_paths})),o=JSON.stringify(t,null,4).replace(/: false/g,": False").replace(/: true/g,": True").replace(/: null/g,": None");return`${s}MODELS = ${o}`}generateFunctions(){return`# ============================================
...
def make_api_call(model_config, prompt, timeout=90):
    ...
    try:
        ...
        return {
            ...
            'request_body': body,
            'full_response': None
        }`}generateMainFunction(e,r){const s=e.type==="template";return`def main():
    """Run the experiment."""
    ...
    for model_idx, model in enumerate(MODELS):
        ...
        ${s?`for combo_idx, combination in enumerate(all_combinations):
            ...
            test_counter += 1
            # Show progress
            print(f"[{len(results) + 1}/{total_tests}] Testing {model['name']} with {variable_values}")`:`# Single test per model
            # Skip if we already completed this test
    ...
    main()`}}class I{getTemplate(){return{id:"production",name:"Production Script (Professional)",description:"Production-grade Python script with SQLite persistence, professional retries, progress bars, and multiple export formats. Best for research and large experiments.",pythonVersion:"3.8",requiredImports:["httpx","aiometer","aiosqlite","jmespath","tenacity","tqdm","pandas"],templateCode:"PRODUCTION_TEMPLATE",supportsAnalysis:!0,supportsVisualization:!1}}generateScript(e,r,s={}){const{includeComments:t=!0}=s,o=this.buildResolvedConfigurations(e),a=this.discoverParameterColumns(o);return[this.generateHeader(e,t),this.generateImports(),this.generateSettings(e,t),this.generateParameterColumns(a,t),this.generateModelConfigs(o,t),this.generateDatabaseFunctions(a),this.generateApiCallFunction(),this.generateMainFunction(e),this.generateExecutor()].filter(Boolean).join(`

`)}buildResolvedConfigurations(e){const r=[],s=new Map,t=new Map;for(const o of e.configurations){const a=o.modelSnapshot?.displayName||o.modelId;s.set(a,(s.get(a)||0)+1),t.set(a,0)}for(let o=0;o<e.configurations.length;o++){const a=e.configurations[o],n=a.providerSnapshot;if(!n)continue;const p=n.api?.baseUrl||"https://api.example.com",u=n.api?.endpoint||"/v1/api",l={"Content-Type":"application/json",...n.headers};n.auth?.type==="bearer"?l.Authorization="Bearer {{API_KEY}}":n.auth?.type==="header"&&n.auth.header&&(l[n.auth.header]="{{API_KEY}}");const d={model:a.modelId},i=n.bodyConstruction;i?.promptField?i.wrapAsArray?d[i.promptField]=[{role:i.messageRole||"user",content:"{{PROMPT}}"}]:d[i.promptField]="{{PROMPT}}":d.prompt="{{PROMPT}}",Object.assign(d,a.parameters);const h=a.provider.split("-")[0],c=a.modelSnapshot?.displayName||a.modelId;let m=c;if(s.get(c)>1){const y=t.get(c)+1;t.set(c,y),m=`${c} [Config ${y}]`}r.push({config_index:o,name:c,display_name:m,provider:h,url:p+u,headers:l,body:d,parameters:a.parameters||{},extract_paths:this.getExtractPaths(a)})}return r}getExtractPaths(e){const r=e.providerSnapshot;if(!r)return["response"];const s=r.responseModes?.text;if(!s)return["response"];const t=s.responseTransform;if(!t)return["response"];const o=[];return t.contentPath&&o.push(t.contentPath),t.fallbackPaths&&o.push(...t.fallbackPaths),o.length>0?o:["response"]}discoverParameterColumns(e){const r=new Set;for(const s of e)s.parameters&&Object.keys(s.parameters).forEach(t=>r.add(t));return Array.from(r).sort()}generateParameterColumns(e,r){const s=r?`# === DISCOVERED PARAMETERS ===
...
import functools
import pandas as pd
from pathlib import Path`}generateSettings(e,r){const s=r?`# === WHAT WE'RE TESTING ===
`:"";let t="";if(e.type==="template"&&e.templateConfig){const{template:o,variables:a}=e.templateConfig,n={};for(const[u,l]of Object.entries(a))"snapshot"in l&&l.snapshot?n[u]=l.snapshot.items.map(d=>d.value):l.type==="value"&&l.values&&(n[u]=[...l.values]);const p=JSON.stringify(n,null,4).replace(/: false/g,": False").replace(/: true/g,": True").replace(/: null/g,": None");t=`PROMPT_TEMPLATE = ${JSON.stringify(o)}
VARIABLES = ${p}`}else t='PROMPT = "Your test prompt here"';return`${s}${t}`}generateModelConfigs(e,r){const s=r?`
# === MODELS TO TEST ===
`:"",t=e.map(a=>({config_index:a.config_index,name:a.name,display_name:a.display_name,provider:a.provider,url:a.url,headers:a.headers,body:a.body,parameters:a.parameters,extract_paths:a.extract_paths})),o=JSON.stringify(t,null,4).replace(/: false/g,": False").replace(/: true/g,": True").replace(/: null/g,": None");return`${s}MODELS = ${o}`}generateDatabaseFunctions(e){return`# === DATABASE ===
...
async def save_result(db, task_id, model_config, prompt, variable_values, request_payload, response, extracted, success, error):
    ...
    parameter_values = [model_config['parameters'].get(p, '') for p in PARAMETER_COLUMNS]
...
async def call_api(client, db, task_id, model_config, prompt, variable_values):
    ...
    try:
        ...
        try:
            ...
            await save_result(db, task_id, model_config, prompt, variable_values, body, raw_response, None, False, error_msg)
            fail_count += 1
            if progress_bar:
        ...
        if response_data and response_data.get('error') is not None:
            ...
            await save_result(db, task_id, model_config, prompt, variable_values, body, response_data, None, False, error_msg)
            fail_count += 1
            if progress_bar:
        ...
        if success:
            ...
            if progress_bar:
                paths_str = ', '.join(attempted_paths[:3])  # Show first 3 paths tried
                if len(attempted_paths) > 3:
                    paths_str += f' (+{len(attempted_paths)-3} more)'
                progress_bar.write(f"⚠️  {model_config['name']}: Couldn't extract content. Tried: {paths_str}")

        await save_result(db, task_id, model_config, prompt, variable_values, body, response_data, str(extracted) if extracted else None, success,
        ...
        raise`}generateMainFunction(e){return`def parse_args():
    """Parse command line arguments."""
...
if __name__ == "__main__":
    asyncio.run(main())`}}function N(f){switch(f){case"simple":return new S;case"production":return new I;case"minimal":case"basic":case"reproduction":case"analysis":return _.info(`Legacy template '${f}' requested, using 'simple' instead`),new S;default:return _.warn(`Unknown template: ${f}, defaulting to 'simple'`),new S}}function j(){return[new S().getTemplate(),new I().getTemplate()]}class F{async generateScript(e,r={}){const s=Date.now();_.info("EXPORT_DEBUG: PYTHON_GENERATION START",{trialId:e,template:r.template||"minimal",startTime:s});try{_.info("EXPORT_DEBUG: PYTHON_GENERATION LOADING_TRIAL",{trialId:e});const t=await L.loadTrialForExport(e);if(!t.ok){const y=t.error;return{ok:!1,error:new x("GENERATION_FAILED",`Failed to load trial data: ${y.message}`,{trialId:e,originalError:y})}}const{trial:o,apiCalls:a}=t.value,n=this.validateCodeGenOptions(r);if(!n.isValid)return{ok:!1,error:new x("INVALID_FIELDS",`Code generation validation failed: ${n.errors.join(", ")}`,{errors:n.errors})};const p=r.template||"minimal";_.info("EXPORT_DEBUG: PYTHON_GENERATION GETTING_TEMPLATE",{templateId:p});const u=N(p),l=u.getTemplate();_.info("EXPORT_DEBUG: PYTHON_GENERATION GENERATING_SCRIPT",{templateId:p,apiCallCount:a.length});const d=Date.now(),i=u.generateScript(o,a,{includeComments:r.includeComments!==!1});_.info("EXPORT_DEBUG: PYTHON_GENERATION SCRIPT_GENERATED",{durationMs:Date.now()-d,codeLength:i.length,estimatedBytes:A(i)});const h=Date.now()-s,m={filename:this.generateScriptFilename(o.name,p),mimeType:"text/x-python",data:i,metadata:{trialName:o.name,trialId:o.id,exportDate:new Date,callCount:a.length,fieldCount:0,format:"python",estimatedSizeBytes:A(i)},template:p,pythonVersion:l.pythonVersion,requiredPackages:this.extractRequiredPackages(l,r),codeLength:i.length,generationTimeMs:h};return _.info("EXPORT_DEBUG: PYTHON_GENERATION SUCCESS",{trialId:e,templateId:p,filename:m.filename,codeLength:m.codeLength,generationTimeMs:m.generationTimeMs,requiredPackages:m.requiredPackages}),{ok:!0,value:m}}catch(t){const o=t instanceof Error?t:new Error(String(t));return _.info("EXPORT_DEBUG: PYTHON_GENERATION UNEXPECTED_ERROR",o,{trialId:e,template:r.template||"minimal",generationTimeMs:Date.now()-s}),{ok:!1,error:new x("GENERATION_FAILED",`Python script generation failed: ${o.message}`,{trialId:e,options:r,originalError:t,generationTimeMs:Date.now()-s})}}}getAvailableTemplates(){return j()}validateCodeGenOptions(e){const r=[],s=[];if(e.template){const t=this.getAvailableTemplates().map(o=>o.id);t.includes(e.template)||r.push(`Invalid template: ${e.template}. Available templates: ${t.join(", ")}`)}if(e.pythonVersion){const t=["3.8","3.9","3.10","3.11"];t.includes(e.pythonVersion)||s.push(`Python version ${e.pythonVersion} not explicitly supported. Supported versions: ${t.join(", ")}`)}if(e.includeVisualization){const t=e.template||"minimal";N(t).getTemplate().supportsVisualization||s.push(`Template '${t}' does not support built-in visualization. You can add your own visualization code to the exported script.`)}return{isValid:r.length===0,errors:r,warnings:s}}generateScriptFilename(e,r){const s=e.replace(/[^a-zA-Z0-9\s-_]/g,"").replace(/\s+/g,"_").toLowerCase().substring(0,30).replace(/^_+|_+$/g,"")||"trial",t=r==="minimal"?"":`_${r}`,o=new Date().toISOString().split("T")[0];return`${s}${t}_${o}.py`}extractRequiredPackages(e,r){return["json","requests","itertools","csv"]}}const D=new F;let w=null;async function B(){return w||(w=(await M(async()=>{const{default:f}=await import("./jszip.min-C0L9SgBO.js").then(e=>e.j);return{default:f}},__vite__mapDeps([0,1,2,3,4]))).default),w}class ${constructor(){k(this,"trialBundleService",new q)}async exportBundle(e,r){try{_.info("EXPORT_DEBUG: REPRO_BUNDLE START",{trialId:e,bundleType:r.bundleType});const s=await D.generateScript(e,{template:r.template||"simple",includeComments:r.includeComments!==!1});if(!s.ok)return{ok:!1,error:new x("GENERATION_FAILED",s.error?.message||"Failed to generate Python script")};if(r.bundleType==="script-only")return{ok:!0,value:{zipBlob:new Blob([s.value.data],{type:"text/x-python"}),filename:s.value.filename,sizeBytes:s.value.metadata.estimatedSizeBytes,contents:["experiment.py"]}};const t=await B(),o=new t,a=[];o.file("experiment.py",s.value.data),a.push("experiment.py");const n=await this.trialBundleService.exportTrialBundle(e,{includeResults:!0,includeApiCalls:!0,skipDownload:!0});if(n.ok?(o.file("trial_config.json",JSON.stringify(n.value.bundle,null,2)),a.push("trial_config.json")):_.warn("Failed to export trial bundle:",n.error),r.bundleType==="script-config-data"){const h=o.folder("data"),c=await R.getAvailableFields(e);let m=[];if(c.ok){const v=c.value.standardFields.map(g=>g.id).filter(g=>g!=="parameters"&&g!=="variables"),E=c.value.parameterFields.map(g=>g.id),b=(c.value.variableFields||[]).filter(g=>!g.id.startsWith("source_")).map(g=>g.id);m=[...v,...E,...b],_.info("EXPORT_DEBUG: REPRO_BUNDLE using all fields",{total:m.length,standard:v.length,params:E.length,vars:b.length})}else _.warn("Failed to get available fields, using defaults");const y=[{format:"csv",filename:"results.csv"},{format:"excel",filename:"results.xlsx"},{format:"jsonl",filename:"results.jsonl"}];for(const{format:v,filename:E}of y){const b=await R.exportTrialData(e,{format:v,fields:m.length>0?m:void 0,includeHeaders:!0,addBOM:v==="csv"||v==="excel"});if(b.ok){if(v==="excel"&&typeof b.value.data=="string"){const g=atob(b.value.data),P=new Uint8Array(g.length);for(let T=0;T<g.length;T++)P[T]=g.charCodeAt(T);h.file(E,P)}else h.file(E,b.value.data);a.push(`data/${E}`)}else _.warn(`Failed to export ${v}:`,b.error)}}const p=this.generateReadme(r.bundleType,s.value.metadata.trialName);o.file("README.md",p),a.push("README.md");const u=await o.generateAsync({type:"blob",compression:"DEFLATE",compressionOptions:{level:6}}),l=new Date().toISOString().split("T")[0].replace(/-/g,""),d=r.bundleType==="script-config-data"?"_complete":"_config",i=`${s.value.metadata.trialName.replace(/[^a-zA-Z0-9_-]/g,"_")}_repro${d}_${l}.zip`;return{ok:!0,value:{zipBlob:u,filename:i,sizeBytes:u.size,contents:a}}}catch(s){return _.error("EXPORT_DEBUG: REPRO_BUNDLE ERROR",s),{ok:!1,error:new x("GENERATION_FAILED",s instanceof Error?s.message:"Unknown error creating bundle")}}}generateReadme(e,r){const s=new Date().toISOString();let t="";switch(e){case"script-config":t="\n## Contents\n- `experiment.py` - Minimal Python script to reproduce the experiment\n- `trial_config.json` - Complete trial configuration including templates, variables, and model settings\n";break;case"script-config-data":t="\n## Contents\n- `experiment.py` - Minimal Python script to reproduce the experiment  \n- `trial_config.json` - Complete trial configuration\n- `data/results.csv` - Full experimental results in CSV format\n- `data/results.xlsx` - Full experimental results in Excel format\n- `data/results.jsonl` - Full experimental results in JSONL format (one JSON object per line)\n";break}return`# Reproducibility Bundle: ${r}

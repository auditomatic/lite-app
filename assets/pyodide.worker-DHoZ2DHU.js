var p=Object.defineProperty;var y=(d,t,e)=>t in d?p(d,t,{enumerable:!0,configurable:!0,writable:!0,value:e}):d[t]=e;var s=(d,t,e)=>y(d,typeof t!="symbol"?t+"":t,e);self.importScripts("https://cdn.jsdelivr.net/pyodide/v0.26.4/full/pyodide.js");class h{constructor(){s(this,"pyodide",null);s(this,"isInitialized",!1);s(this,"initPromise",null);s(this,"stdoutBuffer",[]);s(this,"stderrBuffer",[]);s(this,"interruptBuffer",null);s(this,"memoryMonitorInterval",null);s(this,"executionCount",0);self.addEventListener("message",this.handleMessage.bind(this))}async handleMessage(t){const e=t.data;try{switch(e.type){case"init":await this.initialize(e.indexURL,e.packages);break;case"execute":await this.execute(e);break;case"loadPackages":await this.loadPackages(e.packages);break;case"setupNLTK":await this.setupNLTK(e.dataPath);break;case"interrupt":this.interrupt();break;default:this.sendError("Unknown message type",void 0)}}catch(i){this.sendError(i instanceof Error?i.message:"Unknown error",e.id)}}async initialize(t,e){if(this.isInitialized){this.send({type:"ready"});return}if(this.initPromise){await this.initPromise;return}this.initPromise=this.doInitialize(t,e),await this.initPromise}async doInitialize(t,e){try{const i=r=>{this.stdoutBuffer.push(r),this.send({type:"stdout",data:r})},a=r=>{this.stderrBuffer.push(r),this.send({type:"stderr",data:r})};this.pyodide=await loadPyodide({stdout:i,stderr:a,fullStdLib:!1}),self.crossOriginIsolated&&(this.interruptBuffer=new Uint8Array(new SharedArrayBuffer(1)),this.pyodide.setInterruptBuffer(this.interruptBuffer));const c=e||["micropip","numpy","regex"];console.log("[Pyodide Worker] Loading micropip, numpy, packaging, regex");try{await this.pyodide.loadPackage(c),console.log("[Pyodide Worker] Packages loaded successfully")}catch(r){console.error("[Pyodide Worker] Failed to load packages:",r)}await this.pyodide.runPythonAsync(`
      ...
      `),this.send({type:"nltk_ready"})}catch(e){throw new Error(`NLTK setup failed: ${e}`)}}async execute(t){if(!this.pyodide)throw new Error("Pyodide not initialized");const{id:e,code:i,functionName:a,args:c}=t;try{this.stdoutBuffer=[],this.stderrBuffer=[],await this.pyodide.loadPackagesFromImports(i);const r=`_parser_input_${e}`;try{const u=c[0];if(this.pyodide.globals.set(r,u),this.pyodide.globals.get(r)===void 0)throw new Error(`Failed to verify ${r} was set in globals`)}catch(u){throw console.error(`[Pyodide Worker] Failed to set ${r} in Pyodide globals:`,u,"args[0]:",c[0]),new Error(`Failed to set parser input: ${u}`)}const l=`
import sys
import json
import traceback
import io
...
_parser_result
      `,n=await this.pyodide.runPythonAsync(l);this.pyodide.globals.delete(r);let o=n;n&&typeof n.toJs=="function"&&(o=n.toJs({dict_converter:Object.fromEntries})),o.stdout&&this.send({type:"stdout",id:e,data:o.stdout}),o.stderr&&this.send({type:"stderr",id:e,data:o.stderr}),o.success?this.send({type:"result",id:e,data:o.value}):(console.error("[Pyodide Worker] Python execution error:",{type:o.error_type,message:o.error_message,traceback:o.error}),this.send({type:"error",id:e,error:o.error||o.error_message||"Unknown error"})),this.cleanupExecution(e)}catch(r){let l=r instanceof Error?r.message:String(r);if(this.pyodide)try{const n=await this.pyodide.runPythonAsync(`
import sys
import traceback
